...
test("T7-after: mediana label e tamanho 1.5x via componente", async ({ page }) => {
	...
	const result = await page.evaluate(() => {
		...
		const medianFontSize = parseFloat(
			window.getComputedStyle(median.querySelector('[data-testid="seat-face-num"]')!).fontSize,
		);
		const normalFontSize = parseFloat(
			window.getComputedStyle(normal.querySelector('[data-testid="seat-face-num"]')!).fontSize,
		);

		tempDiv.remove();
		return { medianFontSize, normalFontSize, ratio: medianFontSize / normalFontSize };
	});

	console.log(`[T7-after] median=${result.medianFontSize} normal=${result.normalFontSize} ratio=${result.ratio.toFixed(2)}`);
...
test("T7-after: axe-core 0 serious/critical em /", async ({ page }) => {
	...
	await page.screenshot({ path: "../../screenshots/T7-after.png", fullPage: false });
});