...
- PokerTable: oval fixa `365px` (compact `330px`), feltro `inset 64px 45px`, rail `10px`, radius `180px`, inner highlight + dashed interno. Assentos `84px`, avatar `44px`, played-card `27×38px rotate(12deg)` à frente do assento, voltada ao centro da mesa. Mobile (<700px, não-compact): coluna `520px`, assentos em 2 colunas laterais `65px` e played-card `22×32px`.
...
- **PokerTable:** self = primary fill + ring 2px (ver Shapes). Carta votada fica à frente do avatar, voltada ao centro da mesa: verso (`card-flip` ao votar, classe `--dealt`) antes do reveal, face com o valor (`card-reveal`) depois; o pill do assento (`Votou`/valor) segue como leitura acessível.
...
- **Projéteis (qualquer fase):** os seis projéteis (papel, aviãozinho, pedra, tijolo, tomate e cadeira) têm SVG próprio — sem emoji. Painel próprio junto ao avatar/nome do alvo (sem coss Menu — o Menu travava o bun test no jsdom): hover no desktop, toque alterna, Enter abre; seis opções com ícone, nome e área de toque de 44px, setas navegam, Esc fecha e devolve o foco. Sem seletor de alvo separado, sem menu em si mesmo, assentos vazios ou desconectados. Espectadores podem arremessar e ser alvo pelo nome. Cooldown de 1s por participante (8s para a cadeirada épica; SSOT em `PROJECTILE_COOLDOWN_MS` / `PROJECTILE_CHAIR_COOLDOWN_MS`), com opções desabilitadas e contagem no menu. Voo em arco com origem anônima no centro da mesa até a posição real do alvo (avatar ou nome de espectador), com duração proporcional à distância, sombra de contato no feltro e partículas de impacto na direção do voo; o desfecho (impacto, esquiva ou rebatida) vem do servidor. Overlay sem interação acima dos assentos, animações só de `transform/opacity`; movimento reduzido não cria voo. Sem lista/feed de arremessos — só o voo some sozinho.
