...
- **Containment:** shell global `max-width: 1304px` (`.site-header/.site-main/.site-footer`). Home inner `1240px`. Padding `32px` desktop / `20px` mobile. Steps da home = `repeat(3, 1fr)` com divisórias `border-right` (editorial numerado 01/02/03, não "3 cards iguais"). Gaps verticais via `clamp(3rem, 8vw, 6rem)` (`64–66px` desktop → `40px` hero mobile).
...
- **Hero (home, split assimétrico, nunca centrado):** grid `1.16fr / 1fr`, gap `48px`, padding `38px 0 64px`, `border-bottom`. Copy `max 680px`. Ações: 1 CTA primário (`Criar sala →`, `size=xl`) + 1 text-link sublinhado (`Entrar com código`); sem "Learn more" secundário. Visual: imagem `aspect 1.3`, radius `18px`, `object-fit cover / 65% center` em zona própria — nunca texto sobre imagem. Mobile: 1 coluna, visual `aspect 1.8` abaixo do copy, actions empilham <380px.
