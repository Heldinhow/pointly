...
interface GradientWaveTextProps {
  ...
  speed?: number;
  paused?: boolean;
  /** When true (default), `prefers-reduced-motion: reduce` pauses the rAF loop. */
...
export function GradientWaveText({
  ...
  // Under reduced-motion the rAF loop stays parked unless the consumer opts
  // out via `respectReducedMotion={false}`.
  const effectivePaused = paused || (respectReducedMotion && prefersReducedMotion);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const shouldAnimate = !effectivePaused;
  ...
  }, [speed, effectivePaused, cycles, isInView]);
